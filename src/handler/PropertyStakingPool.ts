...
import {
  ...
  getNewPropertyStakingPoolPosition,
  getNewTokenDeposit,
...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    ...
    const stakingPool =
      loadedStakingPool ?? getNewPropertyStakingPool(event.chainId, event.srcAddress);
    ...
    if (!tokenDeposit) {
      tokenDeposit = getNewTokenDeposit(stakingPoolPosition.id, tokenInformation.id);
    }
    ...
    context.PropertyStakingPoolTransaction.set({
      ...
      transactionType: PropertyStakingPoolTransactionType.DEPOSIT,
...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    ...
    const rewardsPaidOut = event.params.rewardToUser + event.params.rewardToFeeReciever;
    ...
    context.PropertyStakingPool.set({
      ...newStakingPoolData,
      tvl: stakingPool.tvl - withdrawValue,
    });
    ...
    context.PropertyStakingPoolTransaction.set({
      ...
      chainId: event.chainId,
      pool_id: `${event.chainId}-${event.srcAddress}`,
      transactionType: PropertyStakingPoolTransactionType.WITHDRAW,
      transactionHash: event.transaction.hash,
      blockNumber: event.block.number,
      blockTimestamp: event.block.timestamp,
      wallet_id: `${event.chainId}-${event.params.owner}`,
...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    ...
    context.PropertyStakingPoolTransaction.set({
      ...
      chainId: event.chainId,
      pool_id: `${event.chainId}-${event.srcAddress}`,
      transactionType: PropertyStakingPoolTransactionType.REWARD,
      transactionHash: event.transaction.hash,
      blockNumber: event.block.number,
      blockTimestamp: event.block.timestamp,

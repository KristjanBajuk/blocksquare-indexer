...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    const [activeProperties, globalRaw, propertyTokenLoaded] = await Promise.all([
      context.PropertyToken.getWhere({ propertyValuation: { _gt: 0n } }),
      context.Global.getOrCreate(INITIAL_GLOBAL_ENTITY as any),
      context.PropertyToken.get(`${event.chainId}-${event.params.property}`),
    ]);
    const global = globalRaw as unknown as GlobalEntity;
    ...
    if (propertyTokenLoaded) {
      ...
      context.Global.set(globalUpdated as any);
...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    ...
    const [activeProperties, globalRaw2, propertyTokenLoaded] = await Promise.all([
      context.PropertyToken.getWhere({ propertyValuation: { _gt: 0n } }),
      context.Global.getOrCreate(INITIAL_GLOBAL_ENTITY as any),
      context.PropertyToken.get(`${event.chainId}-${event.params.property}`),
    ]);
    const global = globalRaw2 as unknown as GlobalEntity;

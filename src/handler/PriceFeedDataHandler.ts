...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    ...
    /*
    The contractRegister in PriceFeedRegistry.FeedConfirmed collects all the contract
    addresses of the price feeds to group them and so bigger batches of events from the
    RPC can be fetched. This is done to improve the indexing speed.
    Since these contracts have already events before the registry, we need to filter them
    out due to the fact that AssetPair is created in the PriceFeedRegistry. Otherwise

...
BlocksquareToken.Transfer.handler(async ({ event, context }) => {
    ...
    const [bstToken, bstUsdAssetPair, loadedFromTokenHolder, loadedToTokenHolder] =
      await Promise.all([
    ...
    const token =
      bstToken ??
      getNewToken(event.chainId, event.srcAddress, 'BlocksquareToken', 'BST');
    ...
    if (transferFrom === ZeroAddress) {
      tokenUpdated.totalSupply =
        tokenUpdated.totalSupply + event.params._amount;
    } else {
      const fromTokenHolderId = `${event.chainId}-${event.srcAddress}-${event.params._from}`;
    ...
    if (transferTo === ZeroAddress) {
      ...
      if (transferTo === DEAD_ADDRESS)
        tokenUpdated.totalBurnt =
          tokenUpdated.totalBurnt + event.params._amount;

      const toTokenHolderId = `${event.chainId}-${event.srcAddress}-${event.params._to}`;
      let toTokenHolder = loadedToTokenHolder;
      if (!toTokenHolder) {
        toTokenHolder = getNewTokenHolder(
          event.chainId,
          toTokenHolderId,
          event.srcAddress
        );
    ...
    if (bstUsdAssetPair) {
      const marketcapPrecise = BigDecimal(
        formatUnits(tokenUpdated.totalSupply, 18)
      ).times(BigDecimal(bstUsdAssetPair.latestPrice));

      const { formatted: marketcap, formattedBI: marketcapBI } =
        formatTo8Decimals(marketcapPrecise);
    ...
    context.TokenRecord.set(
      getTokenRecord(tokenUpdated, event.block.timestamp)
    );
});

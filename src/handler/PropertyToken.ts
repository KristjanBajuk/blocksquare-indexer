...
PropertyToken.Transfer.handler(async ({ event, context }) => {
  ...
  const [
    ...
    if (!loadedPropertyToken)
      throw new Error(
        'PropertyTokenContract_Transfer_handler: PropertyToken not found'
      );
    ...
    if (transferFrom === ZeroAddress) {
      ...
      if (!fromTokenHolder) {
        ...
        fromTokenHolder = getNewPropertyTokenHolder(
          event.chainId,
          event.srcAddress,
          transferFrom
        );
    ...
    if (transferTo === ZeroAddress) {
      ...
      if (!toTokenHolder) {
        ...
        toTokenHolder = getNewPropertyTokenHolder(
          event.chainId,
          event.srcAddress,
          transferTo
        );
   ...
   const weightedNAVDeviation = calculateWeightedNAVDeviation(propertyTokenUpdated.tokenValuation, propertyTokenUpdated.propertyValuation, propertyTokenUpdated.totalSupply);

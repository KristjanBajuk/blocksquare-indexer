...
export const getNewPropertyTokenRevenueDistribution = (
    ...
    const { totalSupply, propertyValuation } = getSupplyAndValuationForPeriod(normalizeTimestampToSeconds(Number(fromTime)),
        normalizeTimestampToSeconds(Number(toTime)), tokenRecords);
    ...
    return {
        ...
        propertyValuation: propertyValuation !== 0n ? propertyValuation : propertyToken.propertyValuation, // Use the current property valuation if no data is found
